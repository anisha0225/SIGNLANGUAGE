...
class App(tk.Tk):
    def __init__(self):
        ...
        text_to_video_button = ttk.Button(button_frame, text="Text-to-Sign", command=self.show_text_to_video_page)
    ...
    def show_text_to_video_page(self):
        ...
        gif_frame = ttk.Frame(text_to_video_window, padding="10")
        gif_frame.pack(fill=tk.BOTH, expand=True)
        submit_button = ttk.Button(submit_button_frame, text="Submit", command=lambda: self.display_gif(text_entry_var.get(), text_to_video_window, text_entry_var, gif_frame))
        submit_button.pack(side=tk.TOP, pady=5)
        clear_button = ttk.Button(text_to_video_window, text="Clear", command=lambda: self.clear_text_and_gif(text_entry_var, gif_frame))
